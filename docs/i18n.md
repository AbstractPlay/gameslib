...
- Managed locales (`de`, `fr`, `it`, `es-US`) use **English defaults** for `names` (most titles are proper nouns). Run `npm run seed-game-names-managed-locales` after updating English `names` so CI translate does not call Gemini for those keys (`locale-src` entries use `src` = `out`).
