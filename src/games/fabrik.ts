...
export class FabrikGame extends GameBase {
    ...
    constructor(state?: IFabrikState | IAPGameStateV2 | string, variants?: string[]) {
        super();
        if (state === undefined) {
            ...
            if ( ("V" in state) && (state.V === 2) ) {
                state = (this.hydrate(state) as FabrikGame).state();
            }
            this.gameover = (state as IFabrikState).gameover;
            this.winner = [...(state as IFabrikState).winner];
            this.variants = [...(state as IFabrikState).variants];
            this.stack = [...(state as IFabrikState).stack];
        }
        this.load();
    }
