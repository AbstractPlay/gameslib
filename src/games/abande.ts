...
export class AbandeGame extends GameBase {
    ...
    constructor(state?: IAbandeState | IAPGameStateV2 | string, variants?: string[]) {
        super();
        if (state === undefined) {
            ...
            if (typeof state === "string") {
                state = JSON.parse(state, reviver) as IAbandeState|IAPGameStateV2;
            }
            ...
            if ( ("V" in state) && (state.V === 2) ) {
                state = (this.hydrate(state) as AbandeGame).state();
            }
            this.gameover = (state as IAbandeState).gameover;
            this.winner = [...(state as IAbandeState).winner];
            this.variants = (state as IAbandeState).variants;
            this.stack = [...(state as IAbandeState).stack];
        }
        this.load();
    }
