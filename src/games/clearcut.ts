...
export class ClearcutGame extends GameBase {
    ...
    constructor(state?: IClearcutState | IAPGameStateV2 | string) {
        super();
        if (state === undefined) {
            ...
            if ( ("V" in state) && (state.V === 2) ) {
                state = (this.hydrate(state) as ClearcutGame).state();
            }
            this.gameover = (state as IClearcutState).gameover;
            this.winner = [...(state as IClearcutState).winner];
            this.variants = (state as IClearcutState).variants;
            this.stack = [...(state as IClearcutState).stack];
        }
        this.load();
    }
