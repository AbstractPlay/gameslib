        ...
        if (edges.length > 0) {
            ...
            if ( (edges.length === 1) && ( (house.length > 0) || (this.stack.length === 1) ) ) {
                ...
                for (const edge of extensions) {
                    // ignore if edge joins two blocked cells (off the edge of the board)
                    const joined = edge2hexes(edge);
                    let allBlocked = true;
                    for (const {q,r} of joined) {
                        const hex = hexGrid.getHex({q,r});
                        if (hex === undefined) {
                            continue;
                        }
                        const cell = StreetcarGame.coords2algebraic(hex.col, hex.row);
                        if (! StreetcarGame.blockedCells.includes(cell)) {
                            allBlocked = false;
                            break;
                        }
                    ...
                    if (allBlocked) {
                        // console.log(`Edge ${edge.uid} (${JSON.stringify(edge2celldir(edge))}) joins two blocked cells, so can be ignored`);
                        continue;
                    }
