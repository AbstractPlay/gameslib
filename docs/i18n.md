...
- **`resolveGameName(uid, englishFallback?)`** resolves the localized title from the active i18next locale; callers pass `gameinfo.name` as fallback when the key is missing.
