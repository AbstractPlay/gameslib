...
export class CairoCorridorGame extends GameBase {
    ...
    public collectChatLogLine(lines: ChatLogLine[], r: APMoveResult, ctx: ChatLogCollectContext): boolean {
        switch (r.type) {
            case "place":
                this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:PLACE.ccorridor", { where: r.where! });
                return true;
    ...
    public sidebarScores(): IScores[] {
        return [{ name: this.neutralAreaLabel("apgames:status.SCORES"), scores: [this.scores[0], this.scores[1]] }];
    }
