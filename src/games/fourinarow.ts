...
export class FourInARowGame extends InARowBase {
    ...
    public move(m: string, {partial = false, trusted = false} = {}): FourInARowGame {
        ...
        if (m === "pass") {
            ...
            if (this.clear) {
                // Check for win because this takes priority over clear
                if (![1, 2].some(player => this.getWinningLinesMap().get(player as playerid)!.length > 0)) {
                    const clearDirection = this.lineClear();
    ...
    public render(opts?: { altDisplay: string | undefined }): APRenderRep {
        ...
        if (this.results.some(r => r.type === "remove")) {
            const clearDirection = (this.results.find(r => r.type === "remove") as Extract<APMoveResult, { type: 'remove' }>).where;
            if (clearDirection.includes("N")) { markers.push({type:"edge", edge: "N", colour: "#FFA500"}); }
            if (clearDirection.includes("S")) { markers.push({type:"edge", edge: "S", colour: "#FFA500"}); }
            if (clearDirection.includes("E")) { markers.push({type:"edge", edge: "E", colour: "#FFA500"}); }
            if (clearDirection.includes("W")) { markers.push({type:"edge", edge: "W", colour: "#FFA500"}); }
        }
    ...
    public chat(node: string[], player: string, results: APMoveResult[], r: APMoveResult): boolean {
        ...
        switch (r.type) {
            ...
            case "pie":
                ...
                break;
            case "remove":
                node.push(i18next.t(`apresults:DESTROY.fourinarow_${r.where.toLowerCase()}`, { player }));
                resolved = true;
                break;
        }
        return resolved;
