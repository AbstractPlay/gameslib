...
export class DipoleGame extends GameBase {
    ...
    constructor(state?: IDipoleState | IAPGameStateV2 | string, variants?: string[]) {
        super();
        if (state === undefined) {
            ...
            if ( ("V" in state) && (state.V === 2) ) {
                state = (this.hydrate(state) as DipoleGame).state();
            }
            this.gameover = (state as IDipoleState).gameover;
            this.winner = [...(state as IDipoleState).winner];
            this.variants = (state as IDipoleState).variants;
            this.stack = [...(state as IDipoleState).stack];
        }
        this.load();
    }
