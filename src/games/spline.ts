...
export class SplineGame extends GameBase {
    public static readonly gameinfo: APGamesInformation = {
        ...
        categories: ["goal>arrange", "mechanic>place", "mechanic>move", "board>shape>rect", "board>connect>rect", "components>simple>1per", "components>shibumi", "board>3d"],
        flags: [],
        displays: [{ uid: "orb-3d" }],
    };
    ...
    public render(opts?: IRenderOpts): APRenderRep {
        ...
        const rep: APRenderRep =  {
            board: {
                ...
                backFill: this.variants.includes("plus") ? {colour: "#FFA500", opacity: 0.1, type: "board"} : undefined,
            },
            legend,
            pieces: pstr,
        };
    ...
    public collectChatLogLine(lines: ChatLogLine[], r: APMoveResult, ctx: ChatLogCollectContext): boolean {
        switch (r.type) {
            ...
            case "move":
                if (r.how === "drop") {
                    this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:MOVE.ball_drop", {
                        from: r.from!, to: r.to!, count: r.count!,
                    });
                } else {
                    this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:MOVE.ball", {
                        from: r.from!, to: r.to!,
                    });
                }
                return true;
