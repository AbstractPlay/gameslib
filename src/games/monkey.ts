...
export class MonkeyQueenGame extends GameBase {
    ...
    public move(m: string): MonkeyQueenGame {
        if (this.gameover) {
    ...
    protected getVariants(): string[] | undefined {
        ...
        for (const v of this.variants) {
            for (const rec of MonkeyQueenGame.gameinfo.variants!) {
                if (v === rec.uid) {
                    vars.push(rec.name);
                    break;
                }
            ...
            if ( (state._results !== undefined) && (state._results.length > 0) ) {
                ...
                for (const r of state._results) {
                    switch (r.type) {
                        case "move":
                            if (r.what === "queen") {
                                node.push(i18next.t("apresults:MOVE.monkey", {context: "queen", player: name, from: r.from, to: r.to}));
                            } else {
                                node.push(i18next.t("apresults:MOVE.monkey", {context: "single", player: name, from: r.from, to: r.to}));
                            }
                            break;
                        case "capture":
                            if (r.what === "queen") {
