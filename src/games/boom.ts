...
export class BoomGame extends GameBase {
    ...
    constructor(state?: IBoomState | IAPGameStateV2 | string) {
        super();
        if (state === undefined) {
            ...
            if ( ("V" in state) && (state.V === 2) ) {
                state = (this.hydrate(state) as BoomGame).state();
            }
            this.gameover = (state as IBoomState).gameover;
            this.winner = [...(state as IBoomState).winner];
            this.variants = (state as IBoomState).variants;
            this.stack = [...(state as IBoomState).stack];
        }
        this.load();
    }
