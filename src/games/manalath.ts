...
export class ManalathGame extends GameBase {
    ...
    constructor(state?: IManalathState | IAPGameStateV2 | string) {
        super();
        if (state === undefined) {
            ...
            if ( ("V" in state) && (state.V === 2) ) {
                state = (this.hydrate(state) as ManalathGame).state();
            }
            this.gameover = (state as IManalathState).gameover;
            this.winner = [...(state as IManalathState).winner];
            this.variants = (state as IManalathState).variants;
            this.stack = [...(state as IManalathState).stack];
        }
        this.load();
    }
